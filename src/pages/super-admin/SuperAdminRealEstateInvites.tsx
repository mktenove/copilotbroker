...
const SuperAdminRealEstateInvites = () => {
  ...
  const cancelInvite = async (id: string) => { await (supabase.from("invites" as any).update({ status: "cancelled" }).eq("id", id) as any); toast.success("Convite cancelado"); load(); };
  const resendInvite = async (invite: Invite) => {
    ...
    try {
      ...
      toast.success("Novo convite criado!"); load();
                  ...
                  {invites.map(inv => {
                    ...
                    return (
                      <tr key={inv.id} className="border-b border-[#2a2a2e]/50 hover:bg-[#2a2a2e]/30">
                        <td className="py-3 px-4 text-white font-mono text-xs">{inv.email}</td>
                        ...
                        <td className="py-3 px-4"><Badge className={`text-xs ${sc.color} flex items-center gap-1 w-fit`}>{sc.icon}{sc.label}</Badge></td>
                        <td className="py-3 px-4 text-xs text-slate-400 font-mono tabular-nums">{format(new Date(inv.expires_at), "dd/MM/yy HH:mm", { locale: ptBR })}</td>
                        <td className="py-3 px-4 text-xs text-slate-400 font-mono tabular-nums">{format(new Date(inv.created_at), "dd/MM/yy HH:mm", { locale: ptBR })}</td>
