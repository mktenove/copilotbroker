...
const ProntosBrokerPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [projectRes, brokerRes] = await Promise.all([
        supabase.from("projects").select("id").eq("slug", "prontos").maybeSingle(),
  ...
  return (
    <>
      <Helmet>
        <title>Imóveis Prontos — {brokerName || "Corretor"} | Enove</title>
        <meta
          name="description"
          content="Encontre seu imóvel pronto para morar no Vale dos Sinos. Sem espera, sem obra."
        />
        <link rel="canonical" href={`https://onovocondominio.com.br/prontos/${brokerSlug}`} />
...
export default ProntosBrokerPage;
